...
export interface FetchAndExtractReleaseInput {
  ...
  tag: string;
  /** Cache directory root; extracted dist lives at `<cacheDir>/<tag>/`. */
  cacheDir: string;
}
...
export async function fetchAndExtractRelease(
  ...
  const { asset, tag, cacheDir } = input;
  const envOverride = envOverrideFor(asset);
  ...
  try {
    // 2. Download SHA-256 sidecar first — small + cheap. If this fails,
    //    we never burn bandwidth on the multi-MB tarball.
    const expectedSha = await downloadShaSidecar(shaUrl, scratchSha, envOverride);
